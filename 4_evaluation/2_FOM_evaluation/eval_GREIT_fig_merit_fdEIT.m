function params = eval_GREIT_fig_merit_fdEIT(imgs, xyzr_pt)
...
% $Id: eval_GREIT_fig_merit.m 4808 2015-03-29 11:54:47Z bgrychtol-ipa $
mdl = imgs.fwd_model;
...
%  if abs(max(img(:))) < abs(min(img(:))); img= -img; end
   qmi = calc_hm_set_fdEIT( img, 0.995 ); %0.25 original
   if sum(real(img(:)) & qmi(:))<0 ; 
