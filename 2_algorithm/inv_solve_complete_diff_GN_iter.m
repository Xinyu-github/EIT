function img= inv_solve_complete_diff_GN_iter( inv_model, data1, data2)
...
for i=1:nb_iter
    ...
    [RM,J,L]= eidors_cache(@get_fdRM, {inv_model,hp,sol,nb_elem,normalize,with_v2,alpha,mode});
...
%     end
    ...
    if mode == 1 %diff_term + beta*abs_term
        delta_sol=-RM\(J'*[(dPsi-dv);with_v2(2)*(Psi1-data1);with_v2(2)*(Psi2-data2)]+hp*L*sol);
    elseif mode == 2 % pure abs_term
        delta_sol=-RM\(J'*[(Psi1-data1);(Psi2-data2)]+hp*L*sol);
    elseif mode == 3 % 
        delta_sol=-RM\(J'*(Psi1-alpha.*data1+Psi2w-data2)+hp*L*sol);
    end
    ...
    [step_all,step_half] = scale_to_fit_data( img_fwd_model,inv_model, data1, data2,delta_sol,sol,with_v2,alpha,mode);
...
function [RM,J,RtR] = get_fdRM( inv_model,hp,bkgnd,nb_elem,normalize,with_v2,alpha,mode)
...
% be used in the jacobian (cf schriftliche Fassung der Masterarbeit Seite 34)
   ...
   % perform the normalized minimization (cf Masterarbeit Seite 34)
    if normalize==1
        ...
        elseif mode == 2
             J=[J1,zeros(size(J1));zeros(size(J3)),J2];
        elseif mode == 3
            J = [Psi2.*J1./(Psi1.^2),J2./Psi1];
        end
    else
        if mode ==1 
            J=[-J1,J2;with_v2(2)*J1,zeros(size(J1));zeros(size(J1)),with_v2(2)*J3];
        elseif mode == 2
...
function [step_all,step_half] = scale_to_fit_data(img, inv_model,data1, data2,delta_sol,sol, with_v2, alpha,mode)
   ...
   if do_step
      ...
      try
       ...
       clearvars x;
       noncon = @(x)step_constrain(x,sol,delta_sol,nb_elem);
      [step_all,fval1] = fmincon(@(x) to_optimize_all(img,data1,data2,delta_sol,sol, x,normalize,with_v2, alpha,mode), ...
                            range(1),[],[],[],[],range(1), x_range2, noncon,opt);
       ...
       x_range2=min(aux(aux>range(1)));
        ...
        clearvars x;
      [step_half,fval2] = fminbnd(@(x) to_optimize_half(img,data1,data2,step_all*delta_sol,sol, x,normalize,with_v2, alpha,mode), ...
...
function [c,ceq]=step_constrain(step,sol,delta_sol,nb_elem)
c = -min(real(calc_cond_change(sol+step*delta_sol,nb_elem)));
ceq=[];
end
