function [SNRmean,imgrs, SE, debug] = my_calc_image_SNR_nostepsize(imdl, hyperparameter,find_min,scale, doPlot)
...
% scale=0.9;
RM = get_fdRM(imdl,scale*s_bkgnd_1,scale*s_bkgnd_2);
nb_elem = size(imdl.fwd_model.elems,1);
% bkgnd = scale*s_bkgnd_1*ones(size(RM,1),nTgts);
...
step_all = ones(1,nTgts);
step_half = step_all;
imgrsNew = [repmat(step_all,nb_elem,1);repmat(step_all.*step_half,nb_elem,1)].*imgrs;
...
tic
for i = 1:nNoise
    noise = randn(size(vi1,1),2*nTgts);
    SNR = norm(vi1-vh1)/norm(noise)/snr;
    vn1 = vi1 + SNR*noise(:,1:nTgts);
    vn2 = vi2 + SNR*noise(:,nTgts+1:end);
%     if weighted_diff(1)
...
%     end
    imgrnNew =  [repmat(step_all,nb_elem,1);repmat(step_all.*step_half,nb_elem,1)].*imgrn;
...
%     i=1;
    ...
    clearvars x;
    bound2 =-min(real(sol(nb_elem+1:end,i)))/min(real(step_all(i)*delta_sol(nb_elem+1:end,i)));
...
%     step_all = repmat(step_all,1,nTgts);
