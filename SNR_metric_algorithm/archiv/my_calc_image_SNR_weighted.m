function [SNRmean,imgrs, SE, debug] = my_calc_image_SNR_weighted(imdl, hyperparameter,find_min, doPlot)
...
RM_inv = RM\J';
if with_v2(1)
    imgrs = -RM_inv*[vd1-vd2; with_v2(2)*-vd1; with_v2(2)*-vd2]-RM\(hp*L*bkgnd);
else
    imgrs = -RM_inv*(vd1-vd2)-RM\(hp*L*bkgnd);
end
imgrs = real(imgrs(nb_elem+1:end,:)-imgrs(1:nb_elem,:));

% shift baseline of reconstructed image background to 0 
% imgrs = imgrs - repmat(mean(imgrs), size(imgrs, 1), 1);
% for ntgts = 1:nTgts
% [bins,edge] = histcounts(imgrs(:,ntgts));
% Sortbins = sort(bins);
% idxmax = find(bins==Sortbins(end));
% idxmax2 = find(bins==Sortbins(end-1));
% trans_value = edge(max([idxmax,idxmax2]));
% imgrs(:,ntgts) = imgrs(:,ntgts)-trans_value;
% end
% flip over when reconstruction in freq2 smaller than in freq1
% imgrs(:,abs(min(imgrs))>max(imgrs)) = -imgrs(:,-min(imgrs)>max(imgrs));
...
if isfield(imdl.hyperparameter, 'SigmaN') && ~isempty(imdl.hyperparameter.SigmaN)
    ...
    SigmaN = speye(size(RM_inv,2));		% assuming uniform and uncorrelated noise
    if with_v2(1)
        Noise_diff = blkdiag(0.25*speye(size(RM_inv,2)/3),with_v2(2)^2*speye(size(RM_inv,2)/3),with_v2(2)^2*2.25*speye(size(RM_inv,2)/3));
        SigmaN = Noise_diff.*speye(size(RM_inv,2));	
    end

end

Signal = diag(Ztilde' * imgrs);    % numerator in equation (9)
VarPerPixel = diag(RM_inv(1:nb_elem,:)*SigmaN*RM_inv(1:nb_elem,:)'...
    +RM_inv(nb_elem+1,:)*SigmaN*RM_inv(nb_elem+1,:)'...
    +2*RM_inv(1:nb_elem,:)*SigmaN*RM_inv(nb_elem+1,:)');  
...
Noise = sqrt(Z' * VarPerPixel);

%RM1 = RM(1:nb_elem,:);
%RM2 = RM(nb_elem+1:2*nb_elem,:);
%RM_ = RM2-RM1;
%VarPerPixel = diag(RM_*SigmaN*RM_');
% Noise = sqrt(Z' * VarPerPixel);
