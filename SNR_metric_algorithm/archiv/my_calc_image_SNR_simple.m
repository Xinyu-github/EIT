function [SNRmean,imgrs, SE, debug] = my_calc_image_SNR_simple(imdl, hyperparameter,find_min, doPlot)
...
bkgnd = scale*ones(size(RM,1),nTgts);
bkgnd(nb_elem+1:end,:) = 1.3*scale*bkgnd(nb_elem+1:end,:);
img = mk_image(imdl.fwd_model, 1.1*scale);
vh1 = fwd_solve(img);
vh1 = vh1.meas;
vh1 = repmat(vh1,1,nTgts);

% img = mk_image(imdl.fwd_model, mean(alpha)*1.3*scale);
img.elem_data = alpha.*bkgnd(nb_elem+1:end,:);
...
if with_v2(1)
    imgrs = -RM*[vh2-vh1-vi2+alpha.*vi1; with_v2(2)*(vh1-vi1); with_v2(2)*(vh2-vi2)];
else
    imgrs = -RM*(vh2-vh1-vi2+alpha.*vi1);
end
% [step_all , step_half] = scale_to_fit_data(img, vh1, vh2,imgrs,bkgnd, with_v2,alpha,nTgts);
% imgrsNew = complex_truncation(bkgnd + [repmat(step_all,nb_elem,1);repmat(step_all.*step_half,nb_elem,1)].*imgrs);
...
nNoise = 20;
snr = 10:40/(nNoise-1):50;
Noise = zeros(nTgts,nNoise);
for i = 1:nNoise
    vn1 = add_noise(snr(i),vi1,vh1);
    vn1 = vn1.meas;
    vn2 = add_noise(snr(i),vi2,vh2);
    vn2 = vn2.meas;
    if with_v2(1)
        imgrn = -RM*[vh2-vn2-vh1+vn1;with_v2(2)*(vh1-vn1);with_v2(2)*(vh2-vn2)];
    else
        imgrn = -RM*(vh2-vn2-vh1+vn1);
    end
    [step_all , step_half] = scale_to_fit_data(img, vh1, vh2,imgrn,bkgnd, with_v2,alpha,nTgts);
    imgrnNew = complex_truncation(bkgnd + [repmat(step_all,nb_elem,1);repmat(step_all.*step_half,nb_elem,1)].*imgrn);
