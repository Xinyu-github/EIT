function [SNRmean,imgrs, SE, debug] = my_calc_image_SNR(imdl, hyperparameter,find_min,scale, doPlot)
...
scale=1;
RM = get_fdRM(imdl,scale*s_bkgnd_1,scale*s_bkgnd_2,mean(alpha));
...
if mode == 1 %diff_term + beta*abs_term
    imgrs = -RM*[(vh2-vh1-vi2+alpha.*vi1); with_v2(2)*(vh1-vi1); with_v2(2)*(vh2_-vi2)];
elseif mode == 2 % pure abs_term
    imgrs=-RM*[(vh1-vi1); (vh2_-vi2)];
elseif mode == 3 % 
...
% increase
%  [step_all , step_half] = scale_to_fit_data(img, vh1, vh2,imgrs,bkgnd, with_v2,alpha,nTgts,1,mode);
...
%     end
    ...
    if mode == 1 %diff_term + beta*abs_term
        imgrn = -RM*[(vh2-vh1-vn2+alpha.*vn1); with_v2(2)*(vh1-vn1); with_v2(2)*(vh2_-vn2)];
    elseif mode == 2 % pure abs_term
        imgrn = -RM*[ (vh1-vn1); (vh2_-vn2)];
    elseif mode == 3 % 
        imgrn = -RM*[vh2+vh1-vn2-alpha.*vn1];
    end
%     if mod(i,5)== 0
%     [step_alln , step_halfn] = scale_to_fit_data(img, vh1, vh2,imgrn,bkgnd, with_v2,alpha,nTgts,1,mode);
%     end
    imgrnNew =  [repmat(step_alln,nb_elem,1);repmat(step_alln.*step_halfn,nb_elem,1)].*imgrn;
